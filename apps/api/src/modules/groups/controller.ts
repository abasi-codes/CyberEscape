...
export async function groupRoutes(app: FastifyInstance) {
  app.post<{ Body: CreateGroupInput }>("/api/v1/groups", {
  ...
  app.get<{ Params: { id: string } }>("/api/v1/groups/:id", {
    preHandler: [authenticate, requireRole("ORG_ADMIN", "MANAGER")],
    handler: async (req, rep) => rep.send(await groupService.getById((req.params as any).id)),
  });

  app.patch<{ Params: { id: string }; Body: UpdateGroupInput }>("/api/v1/groups/:id", {
  ...
  app.delete<{ Params: { id: string } }>("/api/v1/groups/:id", {
  ...
  app.post<{ Params: { id: string }; Body: AssignUsersInput }>("/api/v1/groups/:id/users", {
  ...
  app.delete<{ Params: { id: string; userId: string } }>("/api/v1/groups/:id/users/:userId", {
    preHandler: [authenticate, requireRole("ORG_ADMIN", "MANAGER")],
    handler: async (req, rep) => { const p = req.params as any; rep.send(await groupService.removeUser(p.id, p.userId)); },
  });

  app.get<{ Params: { id: string } }>("/api/v1/groups/:id/members", {
    preHandler: [authenticate, requireRole("ORG_ADMIN", "MANAGER")],
